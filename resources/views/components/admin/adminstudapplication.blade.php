  ...
  <script>
    ...
    const InputFieldManager = {
      ...
      addNewInputField(fieldType) {
        ...
        newInput.innerHTML = `
        <div class="input-content">
            <input type="text" placeholder="${fieldType}" disabled />
        </div>
        <span class="remove-option">×</span>
    `;
    ...
    const CourseDetailsManager = {
      ...
      addNewCourseOption() {
        ...
        `;
          optionsContainer.insertBefore(newOption, document.querySelector('#add-option-btn'));
        }
      },
    ...
    const AcademicDetailsManager = {
      ...
      setupEducationSection() {
        ...
        if (!container || !headerRow || !dropdownIcon || !section) return;
      ...
      addEducationField() {
        ...
        if (fieldName) {
          ...
          newRow.id = `education-row-${this.educationRowCount}`;
          newRow.innerHTML = `
            <div class="education-field">
                <input type="text" placeholder="${fieldName}" disabled>
                <button type="button" class="remove-field-btn">✕</button>
            </div>
        `;
          document.getElementById('education-section').insertBefore(newRow, document.getElementById('add-education-field-btn'));
          newRow.querySelector('.remove-field-btn').addEventListener('click', e => {
            e.stopPropagation();
            newRow.remove();
            this.modified = true;
            FormSubmissionManager.setModifiedSection(this.section);
      ...
      setupAcademicGapSection() {
        ...
        if (!container || !row || !dropdownIcon || !optionsSection) return;
          ...
          addIncomeFieldBtn.addEventListener('click', e => {
            ...
            if (userInput) {
              ...
              fieldContainer.innerHTML = `
                    <input type="text" placeholder="${userInput}" disabled>
                    <button class="delete-field">✕</button>
                `;
    ...
    const DocumentFieldManager = {
      ...
      addNewDocumentField(fieldType, type, containerId) {
        ...
        newBox.innerHTML = `
          <div class="document-name" id="${docId}-document-name" style="display: none">${fieldType}</div>
          <div class="upload-field">
            <span id="${docId}-name" data-original="${fieldType}">${fieldType}</span>
            <span id="${docId}-remove-icon" class="remove-icon" onclick="removeFile('${docId}', '${docId}-name', '${docId}-upload-icon', '${docId}-remove-icon')"><span class="thin-x"></span></span>
            <div class="file-actions">
              <label for="${docId}" class="upload-icon" id="${docId}-upload-icon">
                <img src="assets/images/upload.png" alt="Upload Icon" />
              </label>
              <input type="file" id="${docId}" name="documents[${type}][${fieldType.toLowerCase().replace(/ /g, '_')}]" accept=".jpg, .png, .pdf" onchange="handleFileUpload(event, '${docId}-name', '${docId}-upload-icon', '${docId}-remove-icon')" />
            </div>
          </div>
          <div class="info">
