        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', function () {
              ...
              let requestsData = [];
                ...
                nbfcStatusProgressDetail();

                async function nbfcStatusProgressDetail() {
                    const nbfc_id = "NBFC000520"; // Define NBFC ID

                    try {
                        const response = await fetch("/trace-process", {
                            method: "POST",
                            headers: {
                                'Content-Type': 'application/json',
                                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || ''
                            },
                            body: JSON.stringify({ nbfc_id })
                        });

                        const data = await response.json();

                        if (data.success) {
                            // Clear the arrays to remove old data
                            requestsData.length = 0;
                            proposalsData.length = 0;

                            // Populate the arrays based on returnValues using for...of loop
                        ...
                        console.error("Fetch error:", error); // Handle any errors from the fetch call
                    }
                }
